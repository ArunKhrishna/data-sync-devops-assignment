...
| Resources | requests below limits | requests below limits | CPU: request 2, limit 4. Memory: requests equal limits |
...
| Resource fairness | requests and limits on every pod; production keeps memory requests equal to limits and sets the CPU limit above the request, so bursts are not CFS-throttled at steady state (`docs/DECISIONS.md` #5) |
